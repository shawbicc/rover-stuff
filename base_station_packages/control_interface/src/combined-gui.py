...
def move_forward(obj=None):
    if arming_status.get() == 'armed':
        data = Twist()
        data.linear.x = speed.get()
        data.angular.z = 0.0
        wheel_pub.publish(data)
        rospy.loginfo("Forward, speed: {}".format(speed.get()))
    else:
        rospy.logwarn("Not armed, command rejected")

def move_backward(obj=None):
    if arming_status.get() == 'armed':
        data = Twist()
        data.linear.x = -1.0 * speed.get()
        data.angular.z = 0.0
        wheel_pub.publish(data)
        rospy.loginfo("Backward, speed: {}".format(speed.get()))
    else:
        rospy.logwarn("Not armed, command rejected")

def move_left(obj=None):
    if arming_status.get() == 'armed':
        data = Twist()
        data.linear.x = 0.0
        data.angular.z = -1.0 * speed.get()
        wheel_pub.publish(data)
        rospy.loginfo("Left, speed: {}".format(speed.get()))
    else:
        rospy.logwarn("Not armed, command rejected")

def move_right(obj=None):
    if arming_status.get() == 'armed':
        data = Twist()
        data.linear.x = 0.0
        data.angular.z = speed.get()
        wheel_pub.publish(data)
        rospy.loginfo("Right, speed: {}".format(speed.get()))
    else:
        rospy.logwarn("Not armend, command rejected")
    
def stop_moving(obj=None):
    data = Twist()
    data.linear.x = 0.0
    data.angular.z = 0.0
    wheel_pub.publish(data)
    rospy.loginfo("Stop, last speed: {}".format(speed.get()))
