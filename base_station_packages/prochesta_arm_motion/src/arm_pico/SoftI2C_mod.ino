...
SoftI2C_mod::SoftI2C_mod(uint8_t dataPin, uint8_t clockPin, bool usePullup /*=false*/) {
  sda = dataPin;
  scl = clockPin;
  pullup = usePullup;
}

void SoftI2C_mod::begin(void) {
  rxBufferIndex = 0;
  rxBufferLength = 0;
  error = 0;
...
void SoftI2C_mod::end(void) {
}

void SoftI2C_mod::beginTransmission(uint8_t address) {
  if (isTransmitting) {
...
void SoftI2C_mod::beginTransmission(int address) {
  beginTransmission((uint8_t)address);
}

uint8_t SoftI2C_mod::endTransmission(uint8_t sendStop)
{
...
uint8_t SoftI2C_mod::endTransmission(void)
{
  return endTransmission(true);
}

size_t SoftI2C_mod::write(uint8_t data) {
  if (i2cWrite(data)) {
    return 1;
  } else {
    if (error == 0) error = 3;
...
size_t SoftI2C_mod::write(const uint8_t *data, size_t quantity) {
  size_t progress = 0;
...
uint8_t SoftI2C_mod::requestFrom(uint8_t address, uint8_t quantity,
...
uint8_t SoftI2C_mod::requestFrom(uint8_t address, uint8_t quantity, uint8_t sendStop) {
  return requestFrom((uint8_t)address, (uint8_t)quantity, (uint32_t)0, (uint8_t)0, (uint8_t)sendStop);
}

uint8_t SoftI2C_mod::requestFrom(int address, int quantity, int sendStop) {
...
uint8_t SoftI2C_mod::requestFrom(int address, int quantity) {
  return requestFrom((uint8_t)address, (uint8_t)quantity, (uint8_t)true);
}

int SoftI2C_mod::available(void) {
  return rxBufferLength - rxBufferIndex;
}

int SoftI2C_mod::read(void) {
  int value = -1;
 ...
 */
bool SoftI2C_mod::i2cStart(uint8_t addr) {
  setPinLow(sda);
 ...
 */
bool SoftI2C_mod::i2cStartWait(uint8_t addr) {
 ...
 */
bool SoftI2C_mod::i2cRepStart(uint8_t addr) {
  setPinHigh(sda);
  setPinHigh(scl);
  delayMicroseconds(DELAY);
 ...
 * @return True if the slave replies with an "acknowledge," false otherwise.
 */
bool SoftI2C_mod::i2cWrite(uint8_t value) {
 ...
 */
uint8_t SoftI2C_mod::i2cRead(bool last) {
 ...
 */
void SoftI2C_mod::setPinLow(uint8_t pin) {
  noInterrupts();
  if (pullup) 
 ...
 */
void SoftI2C_mod::setPinHigh(uint8_t pin) {
  noInterrupts();
  if (pullup) 
